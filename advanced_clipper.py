...
{{
    ...
    "continuity_analysis": "剧情连贯性分析"
        ...
        try:
            ...
            if response and response.strip():  # 检查响应不为空
                try:
                    # 解析JSON响应
                    if "```json" in response:
...
"""
    ...
    def cut_video_segment(self, video_path: str, start_time: str, end_time: str, output_path: str, segment_title: str = "") -> bool:
        """剪切视频片段"""
        try:
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    stderr_text = "编码错误，无法显示"
                    stdout_text = "编码错误，无法显示"
